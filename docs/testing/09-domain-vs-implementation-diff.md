...
|---|-----------------------------|----------------------|
| B.1 | Wide totals broken down by physical-run parity (07 §Wide, all rows) | ✅ Covered — `scoring.test.js` ("wide with odd/even extraRuns" and "consecutive wides") enumerates a rotation matrix across wide values |
| B.2 | No-ball with each of 0/1/2/4/6 batsman runs, individually asserting rotation (07 §No Ball) | ✅ Covered — `scoring.test.js` ("restoreStateFromBalls — run mapping (tapRuns)") asserts rotation for batsman runs 0/1/2/4/6 on a no-ball |
| B.3 | Bye/Leg-bye at every run value 1–4, individually asserting rotation (07 §Bye/Leg Bye) | ✅ Covered — `scoring.test.js` ("restoreStateFromBalls — bye/leg-bye rotation matrix") |
| B.3b | Bye/leg-bye should NOT be charged to the bowler's runs (A.5b divergence) | ✅ Covered (pins the current divergent behavior) — `scoring.test.js` ("calculateScore — bowler runs on bye/leg-bye") asserts bye/leg-bye runs ARE currently added to `bowler.runs` |
| B.4 | Non-run-out wicket types entered with non-zero "runs" (A.4's divergence) | ✅ Covered (pins the current permissive behavior) — `scoring.test.js` ("calculateScore — non-run-out wicket types with non-zero runs") |
| B.5 | Run-out placement scenarios distinguishing "crossed vs. not crossed" outcomes (A.5) | ✅ Covered for the *achievable* placements — `scoring.test.js` ("restoreStateFromBalls — run-out placement matrix") now enumerates striker-out at runs=3 and non-striker-out at runs=1/2, alongside the pre-existing 0/1/2 striker-out cases |
| B.6 | Wicket + end-of-over coincidence for every dismissal type (not just the general case) (08 §1.3) | ✅ Covered — `scoring.test.js` ("restoreStateFromBalls — wicket coincides with end of over, all dismissal types") covers caught/lbw/stumped/hit wicket alongside the pre-existing bowled/run-out cases |
| B.7 | Innings/match-ending mid-over for every triggering combination (all-out mid-over, overs-complete mid-over, target-exceeded mid-over) (08 §1.4/1.5) | ✅ Covered — `Scoring.test.jsx` ("Scoring - innings/match ending mid-over") adds an all-out-mid-over case and a target-exceeded-mid-over case; overs-complete inherently only triggers on the over's last ball, so no separate mid-over case applies there |
| B.8 | Team-size increase mid-innings recalculating the all-out threshold on the very next ball (08 §3.1) | ✅ Covered — `Scoring.test.jsx` ("Scoring - mid-innings settings changes recompute thresholds on the next ball") |
| B.9 | Overs increase mid-innings recalculating the overs-complete threshold on the very next ball (08 §3.3) | ✅ Covered — same describe block as B.8 |
| B.10 | Settings changed mid-match not retroactively altering already-recorded balls (08 §3.5) | ✅ Covered — `Scoring.test.jsx` ("Scoring - settings changes do not retroactively alter recorded balls") |
| B.11 | v2 redo after undo, then diverging with a new ball, invalidating stale redo history (already flagged as a gap in doc 03 §4.6) | ✅ Covered — `ScoringV2.test.jsx` ("Redo is disabled until an undo happens, and a new ball clears the redo history") |
| B.12 | Batsman run/four/six crediting under `runMap` (doc 10 rows 10.3/10.4/10.5) — a tap-3-mapped-to-4 should count as a four; a tap-4-mapped-to-5 should not | ✅ Covered — `scoring.test.js` ("calculateScore — batsman runs/fours/sixes use the mapped (recorded) value") |
| B.13 | `disabledRuns` reachability (doc 10 rows 10.10–10.12) — a disabled run has no tap path, but a `runMap` onto it still reaches it; all-runs-disabled degenerate case | ✅ Covered for the reachability half — `Scoring.test.jsx` ("Scoring - custom rule disabledRuns hides the tap in-match") asserts a disabled run's button is absent, including the all-disabled degenerate case. The `runMap`-onto-a-disabled-value combination remains untested. |
